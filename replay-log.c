...
static void usage(void) {
  ...
  fprintf(stderr,
          "\t--replay <device> - replay onto a specific "
          "device\n");
  ...
  fprintf(stderr,
          "\t--start-entry <entry> - start at the given "
          "entry #\n");
  fprintf(stderr, "\t--start-mark <mark> - mark to start from\n");
  fprintf(stderr, "\t--end-mark <mark> - replay to/find the given mark\n");
  fprintf(stderr,
          "\t--find - put replay-log in find mode, will search "
          "based on the other options\n");
  fprintf(stderr,
          "\t--num-entries - print the number of entries in "
          "the log\n");
  fprintf(stderr, "\t--no-discard - don't process discard entries\n");
  fprintf(stderr,
          "\t--fsck - the fsck command to run, must specify "
          "--check\n");
  fprintf(stderr,
          "\t--check [<number>|flush|fua] when to check the "
...
static int should_stop(struct log_write_entry *entry, u64 stop_flags,
  ...
  if (flags & stop_flags) {
    if (!check_mark) return 1;
    if ((flags & LOG_MARK_FLAG) && !strcmp(mark, buf)) return 1;
  }
  return 0;
}
...
static int seek_to_mark(struct log *log, struct log_write_entry *entry,
  ...
  while ((ret = log_seek_next_entry(log, entry, 1)) == 0) {
    if (should_stop(entry, LOG_MARK_FLAG, mark)) break;
  }
  if (ret == 1) {
...
int main(int argc, char **argv) {
  ...
  while ((c = getopt_long(argc, argv, "v", long_options, &opt_index)) >= 0) {
    switch (c) {
      case 'v':
        log_writes_verbose++;
        continue;
    ...
    switch (opt_index) {
      ...
      case START_MARK:
         ...
         * an extra slot for \0.
         */
        start_mark = strndup(optarg, 4097 - sizeof(struct log_write_entry));
        if (!start_mark) {
      ...
      case END_MARK:
         ...
         * an extra slot for \0.
         */
        end_mark = strndup(optarg, 4097 - sizeof(struct log_write_entry));
        if (!end_mark) {
          fprintf(stderr, "Couldn't allocate memory\n");
          exit(1);
        }
  ...
  if (!discard) log->flags |= LOG_IGNORE_DISCARD;
  ...
  if ((fsck_command && !check_mode) || (!fsck_command && check_mode)) usage();
  ...
  if (!replayfile && !dump_only) {
    printf("Log version=%d, sectorsize=%lu, entries=%llu\n", WRITE_LOG_VERSION,
           (unsigned long)log->sectorsize, (unsigned long long)log->nr_entries);
    log_free(log);
    return 0;
  }

  while ((ret = log_replay_next_entry(log, entry, 1, dump_only)) == 0) {
    num_entries++;
    if (fsck_command) {
      if ((check_mode == CHECK_NUMBER) && !(num_entries % check_number))
