...
void log_free(struct log *log) {
  if (log->replayfd >= 0) close(log->replayfd);
  if (log->logfd >= 0) close(log->logfd);
  free(log);
}
...
static int discard_range(struct log *log, u64 start, u64 len) {
  ...
  if (ioctl(log->replayfd, BLKDISCARD, &range) < 0) {
    if (log_writes_verbose)
      printf(
          "replay device doesn't support discard, "
...
int log_discard(struct log *log, struct log_write_entry *entry) {
  ...
  if (log->flags & LOG_IGNORE_DISCARD) return 0;
  ...
  while (size) {
    ...
    if (log->flags & LOG_DISCARD_NOT_SUPP) ret = zero_range(log, start, len);
    if (ret) return -1;
    size -= len;
    start += len;
  }
...
struct flags_to_str_entry {
  ...
  const char *str;
    ...
    DEFINE_LOG_FLAGS_STR_ENTRY(FLUSH), DEFINE_LOG_FLAGS_STR_ENTRY(FUA),
    DEFINE_LOG_FLAGS_STR_ENTRY(DISCARD), DEFINE_LOG_FLAGS_STR_ENTRY(MARK),
...
static void entry_flags_to_str(u64 flags, char *buf) {
  ...
  for (i = 0; i < ARRAY_SIZE(log_flags_table); i++) {
    if (flags & log_flags_table[i].flags) {
      if (!empty) strncat(buf, "|", LOG_FLAGS_BUF_SIZE);
      empty = 0;
  ...
  if (flags) {
    if (!empty) strncat(buf, "|", LOG_FLAGS_BUF_SIZE);
    empty = 0;
    left_len = LOG_FLAGS_BUF_SIZE - strnlen(buf, LOG_FLAGS_BUF_SIZE);
    if (left_len > 0)
      snprintf(buf + strnlen(buf, LOG_FLAGS_BUF_SIZE), left_len, "UNKNOWN.%llu",
               flags);
  }
  if (empty) strncpy(buf, "NONE", LOG_FLAGS_BUF_SIZE);
}
 ...
 * if this is set.
 * @dump_only: don't replay the log, just dump it out
 *
...
int log_replay_next_entry(struct log *log, struct log_write_entry *entry,
  ...
  u64 flags;
  size_t read_size =
      read_data ? log->sectorsize : sizeof(struct log_write_entry);
  char *buf;
  char flags_buf[LOG_FLAGS_BUF_SIZE];
  ssize_t ret;
  off_t offset;

  if (log->cur_entry >= log->nr_entries) return 1;
  ...
  if (read_size < log->sectorsize) {
    if (lseek(log->logfd, log->sectorsize - sizeof(struct log_write_entry),
              SEEK_CUR) == (off_t)-1) {
  ...
  if (log_writes_verbose || dump_only)
    printf("replaying %d: sector %llu, size %llu, flags %llu(%s)\n",
           (int)log->cur_entry - 1,
           (unsigned long long)le64_to_cpu(entry->sector),
           (unsigned long long)size, (unsigned long long)flags, flags_buf);
  if (!size || dump_only) return 0;

  if (flags & LOG_DISCARD_FLAG) return log_discard(log, entry);
...
int log_seek_entry(struct log *log, u64 entry_num) {
  ...
  for (i = log->cur_entry; i < entry_num; i++) {
    ...
    if (!(flags & LOG_DISCARD_FLAG))
      seek_size += le64_to_cpu(entry.nr_sectors) * log->sectorsize;
...
int log_seek_next_entry(struct log *log, struct log_write_entry *entry,
  ...
  if (!read_size || (flags & LOG_DISCARD_FLAG)) return 0;
